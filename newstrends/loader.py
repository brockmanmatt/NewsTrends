...
__all__ = ['article_holder', 'article_holder', 'CoverageTrends', 'article_holder']
...
# Cell
def CoverageTrends(publications:[str] = [], dateStart:str=None, dateEnd:str=None, lastN:int=None, verbose=False) -> []:
...
class article_holder(article_holder):
    ...
    def load_articles(self, engine=CoverageTrends, publications:[str] = [], dateStart:str=None, dateEnd:str=None, lastN:int=None, verbose=False) -> []:
        ...
        tmp["tokens"] = tmp["quickReplace"].apply(lambda x: [stemmer.stem(y) for y in x.split() if len (y) > 2])
